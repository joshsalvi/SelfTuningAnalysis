...
% Import logdata
logdata = importdata(logfile);  % logdata.data contains log data of interest
...
for i = 1:a
data=importdata(file{i});   % initial import
...
for j = 1:a
    for i = 1:(logdata.data(1,iter_loc))
        ...
        Xo(:,i,j) = data0(:,(1+logdata.data(1,iter_loc)+i),j);  % stimulus piezo
    end
end
